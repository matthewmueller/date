...
var rMonths = /^((\d{1,2})\s*(st|nd|rd|th))\s(day\s)?(of\s)?((?:(?:this|next|last)\s)?month|january|february|march|april|may|june|july|august|september|october|november|december)/i
var rPast = /\b(last|yesterday|ago|today)\b/
...
parser.prototype.monthByName = function () {
  ...
  if (captures = rMonths.exec(this.str)) {
    ...
    if (monthIndex < 0) {
      monthIndex = this.date.month(0);
      if (month.startsWith('last')) monthIndex = this.date.month(-1);
      if (month.startsWith('next')) monthIndex = this.date.month(1);
    } else {
