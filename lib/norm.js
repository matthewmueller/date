...
function restoreNormal(T) {
  ...
  if (token.match(util.reT)) {
  ...
  } else if (util.has_dt(T) && !util.has_t(T)) {
    // tokens with T that are purely just dt, e.g. week, fortnight
    var dtStr = '',
      keys = _.keys(T.dt),
      dt = T.dt;
    // accumulate dtStr
    for (var i = 0; i < keys.length; i++) {
      var k = keys[i],
        numStr = (dt[k] == 1) ? '' : dt[k].toString() + ' ';
      var lemmaCanon = util.lemma(k).canon;
      // set canon from lemma only if it's [a-zA-Z], else use k
      var canon = lemmaCanon.match(/^\w+$/) ? lemmaCanon : k;
      // set the keys, number, and canonical form of the unit
