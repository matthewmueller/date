...
function norm(str, offset) {
  ...
  try {
    ...
    finalStr = symsToNorm(syms, offset);
...
function ToTOp(L, op, R, offset) {
  // if L missing, always use t first, else R
  if (L == null) {
  ...
  } else if (util.has_t(R)) {
    R = nextAvailable(R, offset)
      // if exist, set to next available
  } else {
    // e.g. if R is 'day', make L to R, set R as nowT
    L = R;
    R = symbol(util.nowT(offset))
  }
 ...
 */
function symsToNorm(syms, offset) {
  var tokens = [];
  for (var i = 0; i < syms.length; i++) {
    var s = syms[i],
