...
var timeUnitOrder = ['y', 'M', 'w', 'd', 'h', 'm', 's', 'ms', 'mer']
var tOrdering = ['y', 'M', 'd', 'h', 'm', 's']
var tFactor = [365, 30, 24, 60, 60]

/**
 * Delimiters for ISO string
 */
...
/**
 * Convert ISO UTC string to T string.
 * @example
...
/**
 * Convert a T string to ISO UTC string, with default filled by nowT().
 * @example
 * TtoISO('t:10M05d14h48m00.000s,dt:')
...
function TtoISO(str) {
  ...
  var resStr = '',
    ISOdelim = ['-', '-', 'T', ':', ':', 'Z'];
  for (var i = 0; i < ISOdelim.length; i++) {
    ...
    resStr += (num + ISOdelim[i])
  }
  return resStr
}
// console.log(TtoISO('t:10M05d14h48m00.010s,dt:'))
...
function carryDown(TO) {
  ...
  for (var i = 0; i < ordering.length; i++) {
    ...
    if (i == ordering.length - 1) {
      // overlong s decimal will be fixed in TtoISO
      break;
    };
...
/**
 * Extract non-null string tokens from an array of symbols
 */
function getTokens(syms) {
  var tokens = []
  for (var i = 0; i < syms.length; i++) {
    if (syms[i]) {
...
/**
 * Get the difference string after removing all parsed tokens from it.
 */
function unparsedStr(str, tokens) {
  var leftover = str;
